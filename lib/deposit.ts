// Client-side USDC deposit for BOOTS packs (devnet). Builds an SPL-token transfer
// from the connected wallet's USDC account to the treasury's, has the wallet
// sign+send it, and returns the signature. The server then verifies that
// signature on-chain before crediting BOOTS — the client is never trusted.
...
// Send the pack's USDC to the treasury. Returns { signature, wallet } — the buyer
// address so the caller can link it to the device on credit.
export async function payForPack(name: WalletName, pack: Pack): Promise<{ signature: string; wallet: string }> {
  ...
  return { signature, wallet: buyer.toString() };
}
